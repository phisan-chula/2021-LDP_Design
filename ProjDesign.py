...
class MapDesign:
    ...
    def GetDEM(self, df_samp ):
        ...
        FILE_DTM = 'DEM/SRTM15p_v21_Thai.tif' 
        with rio.open( FILE_DTM,'r' ) as src:
            dem = src.read(1)
            try:
                df_samp['MSL'] = [msl[0] for msl in src.sample( coords )]
            except:
                RP.PRN(f'*** ERROR reading DTM "{FILE_DTM}"***')
        ...
        #self.FILE_ROI = RESULT_DIR.parents[0].joinpath( self.YAML['KML'] )
        if 'PROFILE' in self.YAML.keys():
            FILE_PFL = Path( self.RESULT_DIR.parents[0], self.YAML['PROFILE'] )
            RP.PRN(f'*** WARNING read profile data "{FILE_PFL}" ***')
            df_pfl = gpd.read_file( FILE_PFL, layer='Profile')
            df_pfl = df_pfl.to_crs('EPSG:4326')
            df_samp = df_samp.sjoin_nearest( df_pfl, how='inner' ,
                            max_distance=None, distance_col='NN_deg' )
            df_samp.rename(mapper={'MSL_right': 'MSL'},axis='columns', inplace=True) 
            #import pdb;pdb.set_trace()
